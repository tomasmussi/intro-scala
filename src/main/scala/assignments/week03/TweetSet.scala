...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val ac1 = this.left.filterAcc(p, acc)
    val ac2 = this.right.filterAcc(p, ac1)
    if (p(this.elem)) ac2.incl(this.elem)
    else ac2
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(k => tw.text.contains(k)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists(k => tw.text.contains(k)))
