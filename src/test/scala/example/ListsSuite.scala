...
class ListsSuite {
  ...
  @Test def `sum same numbers`: Unit = {
  ...
  // Extra tests for max
  @Test def `max of an empty list throws exception`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown")
    } catch {
  ...
  @Test def `max is first`: Unit = {
    assert(max(List(6,5,1)) == 6)
  }

  @Test def `Max duplicate first`: Unit = {
    assert(max(List(6,6,4,1)) == 6)
  }

  @Test def `Max all elements are equal`: Unit = {
    assert(max(List(6,6,6,6,6)) == 6)
  }

  @Test def `Max duplicate last`: Unit = {
    assert(max(List(1,4,5,5)) == 5)
  }

  @Test def `Max is last`: Unit = {
    assert(max(List(1,2,8)) == 8)
  }

  @Test def `Test max in middle`: Unit = {
    assert(max(List(1,2,8,15,4,1,-1,9)) == 15)
  }

  @Test def `Max only 1 element`: Unit = {
    assert(max(List(1)) == 1)
  }
